...
class listen(threading.Thread):
    ...
    def run(self):
        ...
        while 1:
            data, client_address = s_UDP.recvfrom(size)
            if data:
                stream.write(data)
                s_UDP.sendto(data, client_address)
...
exit_bt = tkinter.Button(po, width=20, height=4,
                         text='Exit', bg="#CC0000", command=po.destroy)
